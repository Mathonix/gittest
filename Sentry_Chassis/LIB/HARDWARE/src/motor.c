...
M6020_TypeDef P_stMotor_RudderF = {5};  //底盘左前舵电机
M6020_TypeDef P_stMotor_RudderB = {6};  //底盘右后舵电机
...
void fnMotor_CanRecvDataDecode(CAN_TypeDef* canx,CanRxMsg * msg)
{
	if(canx == CAN1)
	{
		//根据对应ID,进行数据解析
			...
			case MOTOR_RUDDER_F_CAN_ID:
			{
				s_fnM6020_RawDataDecode(&P_stMotor_RudderF,msg);//原始数据解析
			}break;
			
			case MOTOR_RUDDER_B_CAN_ID:
			{
				s_fnM6020_RawDataDecode(&P_stMotor_RudderB,msg);//原始数据解析
			}break;
...
static void s_fnM6020_RawDataDecode(M6020_TypeDef* st_data,CanRxMsg * msg)
{
    ...
    /*根据自定义零点值计算自定义角度值*/    //st_data->RecvData.Custom_Angle = fmod(st_data->RecvData.Raw_Angle - st_data->RecvData.Custom_Angle_Zero + 540.0f, 360.0f) - 180.0f;
    st_data->RecvData.Custom_Angle = fmod(st_data->RecvData.Raw_Angle - st_data->RecvData.Custom_Angle_Zero + 360.0f, 360.0f);
